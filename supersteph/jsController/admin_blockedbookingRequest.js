...
app.controller("BlockedBookingRequestController", function($scope, $http, toaster, $location, $window, $timeout) { 
	...
	function get_booking(){
		$http.post('blockedbookingRequest', $scope.parameters).success(function(data){
	...
	$scope.addAmount = function(){
		...
		$http.post('../addamountBooking', $scope.get).success(function(data){
			...
			if(data.status == 0){
				...
				$window.location.href = base_url+'welcome/blockedbookingRequest/?'+$scope.parameters.page; 
			}
			else{
				toaster.pop("success", data.message, "");
			}
