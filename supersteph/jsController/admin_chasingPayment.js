...
app.controller("ChasingPaymentController", function($scope, $http, toaster, $location, $window, $timeout) { 
	...
	function get_paperworkEmail(){
		$scope.parameters.template='Chasing Payment';
	...
	function get_paperwork(){
		$http.post('chasingPayment', $scope.parameters).success(function(data){
	...
	$scope.assignEmail = function(){
		if(confirm("Do you want to send email?")){
			...
			$http.post('sendPaymentEmail', $scope.get).success(function(data){
				$scope.result = data;
				if(data.status == 0){
					toaster.pop("success", data.message, "");
					$scope.delete_count = 0;
					$window.location.href = base_url+'welcome/chasingPayment';
				}
				else{
