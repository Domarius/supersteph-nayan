...
app.controller("ChasingPaperworkController", function($scope, $http, toaster, $location, $window, $timeout) { 
	...
	function get_paperworkEmail(){
		$scope.parameters.template='Chasing Thank You';
	...
	function get_paperwork(){
		$http.post('chasingThankyou', $scope.parameters).success(function(data){
	...
	$scope.assignEmail = function(){
		if(confirm("Do you want to send email?")){
			...
			$http.post('sendThankyouEmail', $scope.get).success(function(data){
				$scope.result = data;
				if(data.status == 0){
					toaster.pop("success", data.message, "");
					$scope.delete_count = 0;
					$window.location.href = base_url+'welcome/chasingthankyou';
				}
				else{
    ...
    $scope.ignoreEmail = function(){
		if(confirm("Do you want to ignore Booking?")){
			$scope.get.booking_id= $scope.SelectID;
			$http.post('ignoreThankyouEmail', $scope.get).success(function(data){
				$scope.result = data;
				//alert('ok');
				if(data.status == 0){
					//alert(data);
