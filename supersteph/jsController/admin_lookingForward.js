...
app.controller("LookingForwardController", function($scope, $http, toaster, $location, $window, $timeout) { 
	...
	function get_paperworkEmail(){
		$scope.parameters.template='Looking Forward';
	...
	function get_paperwork(){
		$http.post('lookingForward', $scope.parameters).success(function(data){
    ...
    $scope.ignoreassignEmail = function(){
		if(confirm("Do you want to Ignore?")){
			$scope.get.booking_id= $scope.SelectID;
			$http.post('ignoreLookingEmail', $scope.get).success(function(data){
				$scope.result = data;
				if(data.status == 0){
					toaster.pop("success", data.message, "");
					$scope.delete_count = 0;
