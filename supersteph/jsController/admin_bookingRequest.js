...
app.controller("BookingRequestController", function($scope, $http, toaster, $location, $window, $timeout) { 
...
//{"label":"Ignored", "value":"Ignored"},
	$scope.changeBooking = function(booking_status, id){
		
		if(booking_status == 'Cancelled'){
			var reason = 'Admin change manually';
			$http.post('../cancelbookingRequest', {'reason': reason, 'booking_request_id': id}).success(function(data){
				$scope.statusList=data.response;
				get_booking();
			});
		}
		else{
			$http.post('../bookingStatus', {'booking_status': booking_status, 'booking_id': id}).success(function(data){
				$scope.statusList=data.response;
				get_booking();
			});
		}
