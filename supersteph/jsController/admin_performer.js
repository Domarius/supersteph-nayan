...
app.controller("PerformerController", function($scope, $http, toaster, $location, $window, $timeout) { 
	...
	function get_performer(){
		$http.post('performer', $scope.parameters).success(function(data){
    ...
    $scope.blockStatus = function(id, blockStatus){

		var isConfirmed = confirm("Are you sure to change status?");
		if(isConfirmed){
			$http.post('blockPerformer', {'id' : id, 'blockStatus' : blockStatus}).success(function(data){
				$scope.result=data;
				if(data.status == 0){
    ...
    $scope.deletePerformer = function(id){
		...
		if(isConfirmed){
			$http.post('deleteperformer', {'id' : id}).success(function(data){
				$scope.result=data;
				if(data.status == 0){
					toaster.pop("success", data.message, "");
					get_performer();
