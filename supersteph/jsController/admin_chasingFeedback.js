...
app.controller("ChasingFeedbackController", function($scope, $http, toaster, $location, $window, $timeout) { 
	...
	function get_paperworkEmail(){
		$scope.parameters.template='Chasing Feedback';
	...
	function get_paperwork(){
		$http.post('chasingFeedback', $scope.parameters).success(function(data){
	...
	$scope.assignEmail = function(){
		if(confirm("Do you want to send email?")){
			...
			$http.post('sendFeedbackEmail', $scope.get).success(function(data){
				$scope.result = data;
				if(data.status == 0){
					//alert(data.message);
					toaster.pop("success", data.message, "");
					$scope.delete_count = 0;
					$window.location.href = base_url+'welcome/chasingFeedback';
				}
				else{ 
					//alert(data.message);
...
$scope.ignoreEmail = function(){
		if(confirm("Do you want to Ignore Performer Feedback?")){
			$scope.get.booking_id= $scope.SelectID;
			$http.post('ignoreFeedbackEmail', $scope.get).success(function(data){
				$scope.result = data;
				if(data.status == 0){
					//alert(data.message);
					toaster.pop("success", data.message, "");
