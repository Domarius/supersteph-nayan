...
app.controller("ClientController", function($scope, $http, toaster, $location, $window, $timeout) { 
	...
	get_client();
	function get_client(){
		$http.post('client', $scope.parameters).success(function(data){
    ...
    $scope.deleteClient = function(id){
		...
		if(isConfirmed){
			$http.post('deleteclient', {'id' : id}).success(function(data){
				$scope.result=data;
				if(data.status == 0){
